...
class due_payment_unit_check(models.TransientModel):
    ...
    building_unit= fields.Many2many('product.template', string='Filter on unit',domain=[('is_property', '=', True)],
                                         help="Only selected unit will be printed. "
                                              "Leave empty to print all unit.")
    @api.multi
    def check_report(self):
        ...
        return self.env.ref('itsys_real_estate.due_payments_units').report_action([],data=datas)